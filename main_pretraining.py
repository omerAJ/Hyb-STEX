...
def main():
    ...
    ipe_scale = 1.0
    num_graphs = 17
    momentum_scheduler = (ema[0] + i*(ema[1]-ema[0])/(ipe*num_epochs*ipe_scale*num_graphs)
    ...
    tag = r"jepa"
    p = 'testing'
    folder = fr"D:\omer\ST-SSL\logs\{p}"
    ...
    if os.path.exists(log_file) and not p=="testing":
    ...
    def test_step():
        for itr, (data, target) in enumerate(test_loader):
            if args.dataset == "NYCBike1":
                data = data[:, -9:19, :, :]
                
            elif args.dataset == "NYCBike2" or args.dataset == "NYCTaxi" or args.dataset == "BJTaxi": 
                data = data[:, -17:35, :, :]
            ...
            B, T, N, D = data.size()
            for t in range(T):
                _data = data[:, t, :, :].unsqueeze(1)         ## select just one time step
                B, T, N, D = _data.size()
                _data = _data.transpose(1, 2).reshape(B, N, -1)
                B, N, D = _data.size()
                _data = _data.view(B, args.row, args.col, D).to(args.device) ## reshape to 2D grid 
                _data = generateMasks(_data)
                if _data is None:
                    print(f"generateMasks returned None for Epoch, itr: {epoch, train_itr}")
                    continue
                else:
                    imgs, masks_enc, masks_pred = _data
...
# -- Logging
    ...
    for epoch in range(start_epoch, num_epochs):
        ...
        for train_itr, (data, target) in enumerate(train_loader):
            ...
            # data = data[:, b, :, :].squeeze(1)  ## select just one graph
            B, T, N, D = data.size()
            # print("data.shape: ", data.shape)   ## [32, 17, 200, 2]
            for t in range(T):
                # print(t)
                _data = data[:, t, :, :].unsqueeze(1)         ## select just one time step  
                _data = _data.transpose(1, 2).reshape(B, N, -1)  
